#!/usr/bin/env python3
"""Render Early Literacy Adventures LKG P038-P043 with approved artwork only."""
...
SCOPE = [f"EL-LKG-V4-P{number:03d}" for number in range(38, 44)]
...
def parse_pages(value: str | None) -> list[str]:
    ...
    for token in value.split(","):
        ...
        if token not in SCOPE:
            raise ValueError(f"Wave 4 page is outside P038-P043: {token}")
        result.append(token)
    return result
...
def main() -> int:
    ...
    parser.add_argument("--pages", help="Optional comma-separated P038-P043 selection")
    ...
    summary_path = args.evidence_dir / "early-literacy-wave4-render-summary.json"
...
if __name__ == "__main__":
    try:
        ...
        print(f"Early Literacy wave 4 render FAIL: {exc}", file=sys.stderr)
        raise SystemExit(2)
