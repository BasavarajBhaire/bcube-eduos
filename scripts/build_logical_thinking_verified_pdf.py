#!/usr/bin/env python3
"""Build the verified Logical Thinking Adventures LKG review PDF."""
...
def page_paths(pages_dir: Path) -> list[Path]:
    expected = [
        pages_dir / f"LT-LKG-V4-P{number:03d}.png"
    ...
    unexpected = sorted(
        path.name
        for path in pages_dir.glob("LT-LKG-V4-P*.png")
...
def build_pdf(pages: list[Path], output: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="logical-thinking-pdf-") as temporary:
        temporary_dir = Path(temporary)
        document = pdf_canvas.Canvas(str(output), pagesize=A4, pageCompression=1)
        document.setTitle("Logical Thinking Adventures - LKG")
        document.setAuthor("BCube Future Academy")
        document.setSubject("Verified Logical Thinking Adventures learning pages P008-P043")
