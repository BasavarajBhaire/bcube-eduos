#!/usr/bin/env python3
"""Render Early Literacy Adventures LKG P028-P037 with approved artwork only."""
...
SCOPE = [f"EL-LKG-V4-P{number:03d}" for number in range(28, 38)]
...
def parse_pages(value: str | None) -> list[str]:
    ...
    for token in value.split(","):
        ...
        if token not in SCOPE:
            raise ValueError(f"Wave 3 page is outside P028-P037: {token}")
        result.append(token)
    return result
...
def main() -> int:
    ...
    parser.add_argument("--pages", help="Optional comma-separated P028-P037 selection")
    ...
    summary_path = args.evidence_dir / "early-literacy-wave3-render-summary.json"
...
if __name__ == "__main__":
    try:
        ...
        print(f"Early Literacy wave 3 render FAIL: {exc}", file=sys.stderr)
        raise SystemExit(2)
