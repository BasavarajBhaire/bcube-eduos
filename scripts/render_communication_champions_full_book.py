#!/usr/bin/env python3
"""Render Communication Champions LKG P008-P043 from approved asset sheets."""
...
BUILDER = ROOT / "scripts/build_communication_champions_curriculum_first_runtime.py"
COMPOSER = ROOT / "bcube-publishing-sdk/composer/compose_communication_champions_curriculum_first.py"
SCOPE = [f"CC-LKG-V4-P{number:03d}" for number in range(8, 44)]
...
def parse_pages(value: str) -> list[str]:
    ...
    for token in value.split(","):
        ...
        if token.startswith("P") and token[1:].isdigit(): token = f"CC-LKG-V4-P{int(token[1:]):03d}"
...
def main() -> int:
    ...
    summary = {"book": "Communication Champions", "level": "LKG", "scope": scope, "generated": sum(r["status"] == "GENERATED" for r in results), "failed": sum(r["status"] == "FAILED" for r in results), "policy": "Approved content-aligned illustrations; task-specific curriculum layouts; no generic fallback.", "results": results}
    (args.evidence_dir / "communication-champions-render-summary.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
...
if __name__ == "__main__":
    try: raise SystemExit(main())
        ...
        print(f"Communication Champions full-book render FAIL: {exc}", file=sys.stderr); raise SystemExit(2)
